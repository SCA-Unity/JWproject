...
namespace TwoBitMachines.FlareEngine
{
    ...
    public partial class AIFSM : AIBase
    {
        ...
        public override void Execute()
        {
            ...
            {
                _dbgT = Time.time + 1f;
                Debug.Log($"[AIFSM:{name}] Execute running. currentState={(currentState != null ? currentState.stateName : "null")} rootNull={(root == null)} paused={(root != null && root.pause)}");
            }
        ...
        private void RunStates()
        {
            ...
            bool willChange = (currentState != null &&
                root.RunFSM(currentState, position, ref velocity, ref signals.characterDirection, out nextState, true));

            // 0.5초마다 상태/전이 로그 (스팸 방지)
            if (Time.time >= _dbgStateT)
            {
                _dbgStateT = Time.time + 0.5f;
                Debug.Log($"[AIFSM:{name}] state={(currentState != null ? currentState.stateName : "null")} willChange={willChange} nextState={(willChange ? nextState : "-")}");
            }
